...
void random_initialize1(UnionFind_mod* uf,long int n,int num_arrays)
{
	...
	uf->updatesDone = new vector<unordered_map<int, vector<queryParentMapping> > >(num_arrays);
	uf->unionQueriesSent = new vector<unordered_map<long int, vector<long int> > >(num_arrays);
	uf->queriesToBeReplied = new vector<unordered_map<long int, int> >(num_arrays);
	...
	for(i = 0; i < num_arrays; i++)
	{
		unordered_map<long int, vector<long int> > mappingOneProcess;
		// mappingOneProcess.clear();
		(*(uf->unionQueriesSent)).at(i) = mappingOneProcess;
	}
	for(i = 0; i < num_arrays; i++)
	{
		unordered_map<long int, int > mappingOneProcess;
		// mappingOneProcess.clear();
...
void addQuerytoSentQuerySet(long int queryNum,Query q,int process_of_query_y,UnionFind_mod* uf)
{
	...
	(*(uf->unionQueriesSent))[process_of_query_y][queryNum] = currXY;
}
...
void addQueryToQueriesToBeReplied(long int queryNum, int processToSendReplyTo,int currProcess,UnionFind_mod* uf)
{
	...
	(*(uf->queriesToBeReplied))[currProcess][queryNum] = processToSendReplyTo;
}
...
// query sent will be identified by: queryNum -> the (a,b) of this process (i.e. with what x,y this process started), 
// queries to send reply to:  queryNum -> process to send reply to // this will tell to which updatesDone arr does the update needs to get added for further forwarding
// updatesDone will be identified by: vector of  queryNum , parent set for each process stored by each process
...
void unifyOptimized(long int queryNum, Query q, UnionFind_mod* uf, int process_of_x, int process_of_y, long int* num_messages, vector<queryParentMapping> updatesToDo, int queryFromProcess)
{
	...
	{
		// query to be replied
		addQueryToQueriesToBeReplied(queryNum,queryFromProcess,process_of_y,uf);
	}
	...
	if(root_y == its_parent)
	{
		...
		if(root_y < q.query_x)
		{
			...
			if(queryFromProcess != -1)
			{
				addUpdate(queryNum,q.query_x,process_of_y,queryFromProcess,uf);
				// vector<long int> findQuery;
				// findQuery.push_back(queryNum);
				// findQuery.push_back(q.original_x);
				// findQuery.push_back(q.original_y);
				unordered_map<long int, int>::iterator quer_itr = (*(uf->queriesToBeReplied))[process_of_y].find(queryNum);
	...
	else
	{
		...
		{
			if(uf->global_arr[its_parent] != process_of_y)
			{
				*num_messages += 1;
			}
